...
function loadData(sheet,edit=false){
  fetch(SHEET_URL[sheet])
    ...
    .catch(()=>formContent.innerHTML="<p style='color:red;'>ไม่สามารถโหลดข้อมูลได้</p>");
}
...
function saveData(){
  ...
  fetch(SHEET_URL.DATA,{
    ...
    .then(()=>{ alert("บันทึกสำเร็จ"); closeForm(); })
    .catch(()=>alert("เกิดข้อผิดพลาดในการบันทึก"));
}

function editItem(index){ alert("สามารถต่อยอดแก้ไขรายการที่ index: "+index+" ได้ที่นี่"); }
...
function loadReport(){
  ...
  fetch(SHEET_URL.SHOW+"&month="+month)
    ...
    .catch(()=>reportDiv.innerHTML="<p style='color:red;'>ไม่สามารถโหลดรายงานได้</p>");
}
